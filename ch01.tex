...
\begin{proof}
  ...
  \[(\geq \sigma) \cap (\geq \tau) = (\geq (\sigma \cup \tau)) \text{ oder } (\geq \sigma) \cap (\geq \tau) = \emptyset. \]
\end{proof}
...
\begin{satz}
  Sei $X$ ein topologischer Raum mit Basis $\B$. Dann gibt es
...
\begin{proof}
  ...
  = \bigcup_{i \in I} U_i$ eine Vereinigung von Mengen $U_i \in
  \B$. Wir setzen $\hat{F}(U) = \lim F(U_i)$ und prüfen die
  Wohldefiniertheit. Sei $U = \bigcup_{j \in J} V_j$ eine weitere
  Überdeckung von $U$ durch Basismengen $V_j \in \B$.

  %% Durch Übergang zu einer gemeinsamen Verfeinerung $(U_i \cap
  %% V_j)_{i,j}$ können wir annehmen, dass $(V_j)_{j \in J}$ eine
  %% Verfeinerung von $(U_i)_{i \in I}$ ist.
  %% Restriktionen liefern einen Morphismus von Systemen $(F(U_i))_{i \in
  %%   I} \to (F(V_j))_{j \in J}$ und somit auch in den Limites: $\lim
  %% F(U_i) \to \lim F(V_j)$. Diese Abbildung ist bijektiv, da ein
  %% verträgliches Tupel von Schnitten $s_j \in F(V_j)$ nach der
  %% Garbenbedingung auf Basismengen wegen $U_i = \bigcup_{j \in J} (U_i
  %% \cap V_j)$ ein eindeutiges verträgliches Tupel von Schnitten über
  %% $U_i$ definiert. Somit ist der Quasi-Inverse tatsächlich eine
  %% wohldefinierte Zuordnung $F \mapsto \hat{F}$.

  Nun gilt nach der Garbeneigenschaft auf den Basismengen
  ...
  $\hat{F}$ ist tatsächlich eine Garbe, denn falls $U = \bigcup_{i \in
    I} U_i$ eine beliebige Überdeckung durch offene Mengen und $U_i =
  \bigcup_{j} V_{ij}$ jeweils eine Übderdeckung durch Basismengen
  $V_{ij} \in \B$ ist, so gilt

  \[ \hat{F}(U) = \lim_{i.j} F(V_{ij}) = \lim_i F(U_i) \]

  zuerst nach Definition und dann wieder nach der Transitivität von
  Limites und der Garbenbedingung für Basismengen.
...
\begin{satz}
  Der Funktor
  \[
  [\mathcal{K}\op, \EnsX] \to \EnsB \qiso \Ens_{/\mathcal{K} \times X}
  \]
...
\begin{proof}
  ...
  \[ [\mathcal{K}\op, \pEnsX] \iso [\B\op, \Ens] \]
