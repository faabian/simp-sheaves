...
\[
  [\mathcal{K}, \pEnsX] \iso [\mathcal{K} \times \OffX\op, \Ens] \iso [\mathcal{B}\op, \Ens]
\]
...
\begin{satz}
  Sei $X$ ein topologischer Raum mit Basis $\mathcal{B}$. Dann gibt es
  eine Äquivalenz von Kategorien

  \[ \EnsX \to [\mathcal{B}\op, \Ens ] \]

  gegeben durch den Vergissfunktor $\EnsX \to \pEnsX$ und die
  Einschränkung auf $\mathcal{B} \subset \OffX$.
\end{satz}
\begin{proof}
...
\begin{satz}
  Der Funktor
  \[
  [\mathcal{K}\op, \EnsX] \to [\mathcal{B}\op, \Ens] \to \Ens_{/\mathcal{K} \times X}
  \]
