...
\begin{defn} \label{def:sub}
  ...
  \[ X^\S \from f^\S \to Y^\S. \]
\end{defn}
\begin{prop}[\cite{ML}, IX.5.1, \cite{Lore}, 1.13]
...
\begin{proof}
  ...
  \[ \begin{tikzcd}
    c^\S \arrow{d, mapsto} \arrow{l}
    & f^\S \arrow{d, mapsto} \arrow{r}
    & d^\S \arrow{d, mapsto} \\
    F(c, c) \arrow{l}{F(f, \id)
    & F(d, c) \arrow{r}{F(\id, f)
    & F(d, d).
  \end{tikzcd} \]
  Diese Zuordnung $F \mapsto \hat{F}$ ist selbst funktoriell:
  Transformationen $F \Trafo G$ induzieren Transformationen $\hat{F}
  \to \hat{G}$. Kokeile für $F$ und Kokegel für $\hat{F}$ bestehen nun
  beide aus einem Objekt $K$ mit Morphismen $\iota_c: F(c, c) \to K$
  für $c \in C$ und $\iota_f: F(d, c) \to K$ für $f: c \to d$, sodass
  die drei Morphismen $F(d, c) \to K$ übereinstimmen:
...
% TODO subdivision category doch definieren (benötigt in neuer
% Realisierung)
...
\begin{bem} \label{real-model-natural}
  ...
  \Trafo \tilde{R}$ einen Morphismus der Koenden $R \otimes X \to
  \tilde{R} \otimes X$ induziert. Konkret erhalten wir aus unserem
