...
\begin{bem} \label{coend-correct-real}
  ...
  erklären (Koexponentiale, siehe \re{???}). Die Besonderheit
...
\begin{lemma}[Fubini, \cite{Lore}, 1.9]
...
\end{lemma}
\begin{proof}
  Sei $K: D\op \times \to E$ ein Kokeil für $F: C\op \times C \to
  ...
  Morphismen vor das Kokeil-Quadrat von $L(K)$). Nach denselben
  Argumenten definiert ein Kokeil $M$ für $F: (C \times D)\op \times
  (C \times D)$ einen iterierten Kokeil (zunächst für $F(c, c, \cdot,
  \cdot)$ nach der Kokeil-Eigenschaft für $(\id_c, g)$, dann durch
  Ausweitung zu Transformationen $F(c, c, \cdot, \cdot) \Implies M$,
  für die die Kokeil-Eigenschaft von $M$ für $(f, \id_d)$ diejenige in
  der Funktorkategorie impliziert).  Morphismen von Kokeilen
  entsprechen Morphismen iterierter Kokeile und damit entsprechen sich
  auch Koenden alias die universellen Kokeile.
\end{proof}
\begin{bem}
  Ein weniger ad hoc formuliertes Argument würde Kokeile wie in
  \cite{Lore}, 1.12 f. als Kolimites beschreiben und die Aussage damit
  auf die für iterierte Kolimites zurückführen.
\end{bem}
...
\begin{lemma}[Koenden und stetige Funktoren, \cite{Lore}, 1.16]
...
\begin{proof}
  ...
  in \ref{coend-coeq}.
\end{proof}
% TODO ko-/kontravariant, Koenden \to Enden,...
