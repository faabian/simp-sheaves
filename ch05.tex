...
\end{prop}
\begin{proof}
  Wir betrachten wieder mittels des Funktors $r: C \to \Ens^C,
  X \mapsto C(X, \cdot)$ die Kategorie $C \downarrow \! {\scriptstyle
  r} \, F$ der darstellbaren Funktoren über $F$. Wir bezeichnen wieder
  den Kolimes mit $K$ und erhalten mit der universellen Eigenschaft
  eine Tranformation $K \Trafo F$. Diese ist eine Isotransformation,
  wenn sie auf allen Objekten $X \in C$ Bijektionen $KX \iso FX$
  induziert. Nach dem Yoneda-Lemma entsprechen diese den
  Transformationen $C(X, \cdot) \Trafo K$ bzw. $C(X, \cdot) \Trafo
  F$. Diese stehen aber nach der universellen Eigenschaft des Kolimes
  und der Definition der Kategorie $C \downarrow \! {\scriptstyle
  r} \, F$ in Bijektion durch Nachschalten von $K \Trafo F$.
\end{proof}
