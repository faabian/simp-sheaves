...
\begin{bem}
  ...
  \to Y$ diskret. Das interne Hom von $F \Implies G$ lautet $\K(F, G)
  \to \K(X, Y) = \point$ und muss folglich selbst diskret sein. Dies
  ist im Fall, dass $F$ unendlich ist, ein Widerspruch zum folgenden
...
\end{proof}
Wir können sogar zeigen, dass es den Reflektor praktisch nicht gibt.
\begin{prop}
  Der partiell definierte Reflektor $\Top^{[1]} \to \EnsTop$ ist nur
  auf $\EnsTop$ definiert.
\end{prop}
\begin{proof}
  Sei der Reflektor $^+$ definiert auf einem Objekt $F \to X$. Durch
  Testen mit Zielobjekten $\tau Y$, $Y \in \Top$ ist nach dem
  Yoneda-Lemma die Basis von $(F \to X)^+$ wieder (bis auf eindeutigen
  Homöomorphismus) $X$ und die Einheit der Adjunktion liegt über der
  Identität auf $X$. Wegen der eindeutigen Faktorisierung über diese
  Einheit müssen sich nicht nur die gesamten Mengen von kommutativen
  Quadraten von $F \to X$ bzw. $(F \to X)^+$ in eine Garbe
  entsprechen, sondern sogar die Morphismen über jeder Abbildung $f$
  in der Basis. Insbesondere können wir über $X$ testen mit étalen
  Objekten der Form $\Sigma \times U \to X$ für eine diskrete Menge
  $\Sigma$ und offenes $U \open X$. Nach dem Yoneda-Lemma bestimmt
  dies die Schnitte $F(U)$ bzw. $F^+(U)$ eindeutig. Die Restriktionen
  erhalten wir durch
  % TODO problemas beim kein Reflektor-Beweis
\end{proof}
