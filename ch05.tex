...
\begin{satz} \label{enstop-fin-complete}
  ...
  % TODO kovollst. falsch
\end{satz}
\begin{proof}
  Es reicht, die Existenz von Pullbacks (Limites über Winkel) zu
  zeigen. Diese existieren in $\Top^{[1]}$ offensichtlich und ergeben
  für étale $F_i \to X_i$ tatsächlich eine étale Abbildung $\lim_i F_i
  \to \lim_i X_i$. Dies wurde für endliche Produkte in
  \ref{enstop-prod} gezeigt und ist für Einschränkungen auf Teilmengen
  mit der induzierten Topologie klar.
\end{proof}
\begin{bem}
  Die verbleibenden Fragen zur Vollständigkeit und Kovollständigkeit
  von $\EnsTop$ werden in \ref{enstop-not-complete} negativ
  beantwortet.
\end{bem}
...
\begin{proof}
  ...
  $F \to X$ mit $G \to Y$ das Paar $\K(F, G) \times_{\K(F, Y)} \K(X,
  Y) \to \K(X, Y)$, das die Menge der kommutativen Quadrate mit einer
  Topologie ausstattet, mit der Projektion auf den zweiten Faktor als
  Abbildung. Die Adjunktion $\K^{[1]}((F \to X) \times (G \to Y), (H \to
  Z)) \iso \K^{[1]}((F \to X), (G \to Y) \Implies (H \to Z))$ für $(F \to
  X), (G \to Y), (H \to Z) \in \K^{[1]}$ ist dann die Bijektion von
  Faserprodukten
  \[ \K(F \times G, H) \times_{\K(F \times G, Z)} \K(X \times Y, Z)
  \iso \K(F, \K(G, H) \times_{\K(G, Z)} \K(Y, Z))
...
\begin{proof} (\cite{TG}, 2.1.40)
  ...
  \]
  wobei letztere Adjunktion sogar die bekannte Äquivalenz von
  Kategorien ist.
\end{proof}
...
\begin{satz}
  Der partiell definierte Koreflektor $^+: \K^{[1]} \to \et \K^{[1]}$ ist nur
  auf $\et \K^{[1]}$ definiert.
\end{satz}
\begin{proof}
  Dies liegt daran, dass die Testobjekte, mit denen wir Objekte in
  $\K^{[1]}$ eindeutig festlegen können, bereits étale sind.

  Konkret gelte also
  \begin{equation} \label{eq:coreflection-adjunction}
    \K^{[1]}((F \to X), (G \to Y)) \iso \et \K^{[1]}((F \to X), (G \to Y)^+)
  \end{equation}
  für alle étalen $F \to X$. Ein Objekt $(H \to Z) \in \K^{[1]}$ ist nach
  dem Yoneda-Lemma (bis auf eindeutigen Isomorphismus) eindeutig
  festgelegt durch seine Yoneda-Einbettung $\K^{[1]}(\cdot, (H \to
  Z))$. Wir behaupten, dass sogar die Funktoren $\K^{[1]}(\iota \cdot, (H
  \to Z)) = \K(\cdot, Z)$ und $\K^{[1]}(\tau \cdot, (H \to Z)) = \K(\cdot,
  H): \K \to \Ens$ und ihre aus $\iota \cdot \Implies \tau \cdot$
  entstehende Transformation ausreichen, um $H \to Z$ eindeutig
  ...
  $\iota X, \tau X \in \et \K^{[1]}$ für alle $X \in \K$, was den Beweis
  abschließt: dann müssen nämlich im Fall von
...
\end{proof}
\begin{kor}
  Es gibt keine kartesisch abgeschlossene Struktur auf $\et \K^{[1]}$,
  falls $\K$ nichtétale Abbildungen enthält.
\end{kor}
...
\begin{prop} \label{enstop-missing-coreflector}
  Der partiell definierte Reflektor $^+: \Top^{[1]} \to \et
  \Top^{[1]}$ ist nur auf $\et \Top^{[1]} = \EnsTop$ definiert.
\end{prop}
\begin{proof}
  Sei der Reflektor $(F \to X)^+ = F^+ \to X^+$ auf $F \to X$
  definiert, d. h. es gebe für alle étalen $G \to Y$ eine natürliche
  Bijektion
  \begin{equation} \label{eq:enstop-reflector}
    \Top^{[1]}((F \to X), (G \to Y))
    \iso \et \Top^{[1]}((F \to X)^+, (G \to Y)).
  \end{equation}
  Dann erhalten wir mit den étalen Testobjekten $\tau Y$ für $Y \in
  \Top$:
  \[ \Top(X, Y) \iso \Top^{[1]}((F \to X), \tau Y)
  \iso \et \Top^{[1]}((F \to X)^+, \tau Y) \iso \Top(X^+, Y).
  \]
  Mit dem Yoneda-Lemma folgt $X^+ \iso X$ und durch Nachschalten $F^+
  \to X^+ \to X$ ohne Einschränkung $X^+ = X$. Die Einheit der
  Adjunktion liefert einen Morphismus $F \to F^+$ von Garben über $X$,
  über den jeder Morphismus in eine Garbe $G \to Y$ eindeutig
  faktorisieren soll.
  % TODO wie weiter? funktioniert das überhaupt?
\end{proof}
