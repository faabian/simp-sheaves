...
\begin{lemma} \label{k-arrow-cart-closed}
  Sei $I$ die Pfeilkategorie $\bullet \to \bullet$ und $\K \subset
  \Top$ eine koreflektive, kartesisch abgeschlossene Kategorie
  topologischer Räume. Dann ist $\K^I$ kartesisch abgeschlossen.
\end{lemma}
\begin{proof}
  Es handelt sich um eine Prägarbenkategorie auf $I\op$ mit Werten in
  ...
  Abbildung. Die Adjunktion $\K^I((F \to X) \times (G \to Y), (H \to
  Z)) \iso \K^I((F \to X), (G \to Y) \Implies (H \to Z))$ für $(F \to
  X), (G \to Y), (H \to Z) \in \K^I$ ist dann die Bijektion von
  Faserprodukten
...
\end{proof}
Im allgemeinen ist das interne Hom in $\K^I$ für $(F \to X)$ und $G
...
über $\K$ als volle Unterkategorie von $\K^I$ auffassen:
\[ \et \K^I := \Ens_{/\K} \subset \K^I. \]
Gäbe es nun einen Koreflektor $^+: \K^I \to \et \K^I$, so hätten wir
für $F, G, H \in \et \K^I$:
\begin{align*}
  \et \K^I(F \times G, H)
  &\iso \K^I(F \times G, H) \\
  &\iso \K^I(F, G \Implies H) \\
  &\iso \et \K^I(F, (G \Implies H)^+),
\end{align*}
mit der kartesisch abgeschlossenen Struktur aus
\ref{k-arrow-cart-closed} im zweiten Schritt. Die Produkte in $\K^I$
und $\et \K^I$ stimmen dabei nach dem folgenden Lemma
überein. Umgekehrt ist, falls ein internes Hom $(G \Implies H)^+$ in
$\et \K^I$ existiert, die Zuordnung $(G \Implies H) \mapsto (G
...
\begin{proof}
  ...
  Homöomorphismus.
\end{proof}
Das Übereinstimmen der Produkte in $\K^I$ und $\et \K^I$ ergibt sich
nun daraus, dass für $(F \to X), (G \to Y) \in \et \K^I$ zunächst das
Produkt $(F \times G \to X \times Y)$ in $\Top$ étale ist, und dann
nach dem Lemma auch das Produkt $k(F \times G) \to k(X \times Y)$ in
$\K^I$ étale und somit in $\et \K^I$ ist.

\begin{satz}
  Der partiell definierte Koreflektor $^+: \K^I \to \et \K^I$ ist nur
  auf $\et \K^I$ definiert.
\end{satz}
\begin{proof}
  Dies liegt daran, dass die Testobjekte, mit denen wir Objekte in
  $\K^I$ eindeutig festlegen können, bereits étale sind.

  Konkret gelte also
  \begin{equation} \label{eq:coreflection-adjunction}
    \K^I((F \to X), (G \to Y)) \iso \et \K^I((F \to X), (G \to Y)^+)
  \end{equation}
  für alle étalen $F \to X$. Ein Objekt $(H \to Z) \in \K^I$ ist nach
  dem Yoneda-Lemma (bis auf eindeutigen Isomorphismus) eindeutig
  festgelegt durch seine Yoneda-Einbettung $\K^I(\cdot, (H \to
  Z))$. Wir behaupten, dass sogar die Funktoren $\K^I(\iota \cdot, (H
  \to Z)) = \K(\cdot, Z)$ und $\K^I(\tau \cdot, (H \to Z)) = \K(\cdot,
  ...
  $\iota X, \tau X \in \et \K^I$ für alle $X \in \K$, was den Beweis
...
\end{proof}
\begin{kor}
  Es gibt keine kartesisch abgeschlossene Struktur auf $\et \K^I$,
...
% TODO ersetze I = Pfeilkategorie durch Ordinal [1]
