...
\begin{satz}
  Es gibt keine kartesisch abgeschlossene Struktur auf $\Ens_{/\Esp}$
  für $\Esp \subset \Top$ eine nichtleere volle Unterkategorie der
  topologischen Räume.
\end{satz}
\begin{proof}
  Im nachgestellten Lemma wird gezeigt, dass Produkt und internes Hom
  in $\Ens_{/\Esp}$ durch Nachschalten des ``Basisfunktors''
  $\Ens_{/\Esp} \to \Esp$ ein Produkt und internes Hom in $\Esp$
  liefern. Sei also $\Esp \subset \Top$ eine kartesisch abgeschlossene
  volle Unterkategorie. Wir konstruieren nun eine kartesisch
  abgeschlossene Struktur auf $\Esp^I$, für $I$ die von $\bullet \to
  \bullet$ erzeugte Kategorie.
