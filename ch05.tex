...
\begin{bem}
  ...
  auch \emph{koreflektiert} (engl. \emph{coreflective}), der
...
\begin{enumerate}
  ...
  \item $\mathcal{I}$ ist abgeschlossen unter endlichen Produkten.
...
\begin{proof}
  ...
  durch faserweise terminale Garben $\tau: X \mapsto
  (X \xrightarrow{\id} X)$ (konstant einelementig). Wir haben
  ...
  Pfeil
  \[ \begin{tikcd}
    \Ens_{/C}(\iota X, U \to L) \arrow{r}{\sim} \arrow{d}{\sim}
    & \Ens_{/C}(\iota X, \tau Y) \times \Ens_{/C}(\iota X, \tau Z) \arrow{d}{\sim}
    C(X, L) \arrow{r}{\sim} C(X, Y) \times C(X, Z).
  \end{tikcd} \]
  ...
  auch, dass $U \iso Y \times Z$ ist und die Abbildung dazwischen der
  eindeutige Isomorphismus zwischen den universellen Objekten. 
...
Im nachgestellten Lemma wird gezeigt, dass Produkt und internes Hom in
$\Ens_{/\Esp}$ durch Nachschalten des ``Basisfunktors'' $\Ens_{/\Esp}
\to \Esp$ ein Produkt und internes Hom in $\Esp$ liefern. Sei also
$\Esp \subset \Top$ eine kartesisch abgeschlossene volle
Unterkategorie. Wir konstruieren nun eine kartesisch abgeschlossene
Struktur auf $\Esp^I$, für $I$ die von $\bullet \to \bullet$ erzeugte
Kategorie.

  Ein Morphismus in $\Esp^I$ ist ein kommutatives Quadrat
  \[ \begin{tikzcd}
    F \arrow[r] \arrow{d}{p} & G \arrow{d}{q} \\
    X \arrow[r] & Y,
  \end{tikzcd} \]
  also ein Element des Faserprodukts von Mengen
  \[ \Esp^I(F \to X, G \to Y) = \Esp(F, G) \times_{\Esp(F, Y)} \Esp(X, Y), \]
  wobei die Abbildungen nach $\Esp(F, Y)$ durch Nachschalten von $q$
  bzw. Vorschalten von $p$ gegeben sind.

  Ist nun $X \Implies Y$ ein internes Hom in $\Esp$, so gilt
  \[ \Esp(\point, X \Implies Y)
     \iso \Esp(\point \times X, Y) \iso \Top(X, Y),
  \]
  d. h. die dem internen Hom-Objekt zugrundeliegende Menge muss
  $\Top(X, Y)$ sein (es ist $\point \in \Esp$ das terminale Objekt, da
  in der vollen Unterkategorie auch alle konstanten Abbildungen
  vorkommen). Dasselbe Argument funktioniert mit $\point \to \point$
  in $\Esp^I$ und das interne Hom-Objekt von $F \to X$ und $G \to Y$
  in $\Esp^I$ hat die dem oberen Objekt zugrundeliegende Menge
  $\Esp^I(F \to X, G \to Y)$ wegen
  \[ \Esp^I(\point \to \point, H \to Z) \iso \Esp(\point, H). \]
  Das untere Objekt ist wie oben erwähnt das interne Hom $X \Implies
  Y$.
...
\begin{lemma}
  Sei $(\times, \Implies)$ eine kartesisch abgeschlossene Struktur
  auf $\Ens_{\Esp}$ für $\Esp$ eine nichtleere volle Unterkategorie
  von $\Top$. Dann liefert das Nachschalten des Basisfunktors $B: (F
  \to X) \mapsto X$ eine kartesisch abgeschlossene Struktur auf
  $\Esp$.
\end{lemma}
\begin{proof}
  Betrachte die Einbettung $\iota: \Esp \inj \Ens_{/\Esp}$ durch
  konstant einelementigen Garben $X \mapsto (X \xrightarrow{\id}
  X)$. Dann ist $B(\iota Y \times \iota Z)$ das Produkt von $Y$ und
  $Z$ in $\Esp$, denn $\Esp^I(F \to X, \iota Y) \iso \Esp(X, Y)$ und
  somit
  \begin{align*}
    \Esp(X, B(\iota Y \times \iota Z))
    &\iso \Esp^I(F \to X, \iota Y \times \iota Z) \\
    &\iso \Esp^I(F \to X, \iota Y) \times \Esp^I(F \to X, \iota Z) \\
    &\iso \Esp(X, Y) \times \Esp(X, Z).
  \end{align*}

  % TODO: korrekt? wieso ist das Produkt konstant einelementiger
  % konstant einelementig?
