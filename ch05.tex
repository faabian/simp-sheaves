...
\[ \begin{tikzcd}
  \Sub(\Delta) \arrow{d}
  & {[n]}^\S \arrow[mapsto]{d}
  & \arrow{l} f^\S \arrow{r} \arrow[mapsto]{d}
  & {[m]}^\S \arrow[mapsto]{d} \\
  \Ens_{/\Top}
  & F_n \times R[n]
  & \arrow{l} Ff^* F_n \times R[n] \arrow{r}
  & F_m \times R[m]
\end{tikzcd}
\]
...
\[ \begin{tikzcd}
  Ff^* F_n \times R[n] \arrow{d} \arrow{r}
  & F_m \times R[n] \arrow{d} \arrow{r}
  & F_m \times R[m] \arrow{d} \\
  X_m \times R[n] \arrow{r}
  & X_m \times R[n] \arrow{r}
  & X_m \times R[m].
\end{tikzcd} \]
Wir erhalten die folgende ``gemischte'' Realisierung:
\begin{prop}
...
\end{prop}
\begin{bem}
  % TODO erklären wieso das für X_n = D_n \times X eine sinnvolle
  % Konstruktion ist
\end{bem}
