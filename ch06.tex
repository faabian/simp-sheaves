...
\item Gibt es einen Rechtsadjungierten $p_*$ zur Verknüpfung
  \[[(X \otimes R)\op, \Ens] \qiso \Ens^{\sk}_{/|X|} \inj \Ens_{/|X|} \]
...
\begin{enumerate}
\item Betrachte die stetigen Abbildungen $q: \coprod_n NX_n \times
  |\Delta^n| \to |X|$ und $r: \coprod_n NX_n \times |\Delta^n| \to
  \coprod_n NX_n \times \blacktriangle^n$. Für $F \in \Ens_{/|X|}$
  konstruieren wir $p_*F$ aus $r_* \,q^*\, F$. Ein nichtdegenerierter
  Simplex $\sigma$ von $X$ entspricht den Punkten $r(q^{-1}(|\sigma|))
  \subset \coprod_n NX_n \times \blacktriangle^n$. Nun setzen wir
  ...
  \to (q^*\, F)_y \iso F_{q(y)} = F_x \] für $r(y) \in r(q^{-1}(x))$.
  
  Anders gesprochen, war die Reihenfolge der ... falsch in \ref{??}.

  $\sigma$ gibt es disjunkte offene Umgebungen $U_i(\sigma)$ der
  Ränder $d_i^* \sigma$ (etwa durch baryzentrische Unterteilung). Ist
  $\sigma$ ein nichtdegenerierter $n$-Simplex von $X$, so bleiben
  diese Mengen in der geometrischen Realisierung $|X|$ bis auf
  Identifikationen der Ränder selbst disjunkt. Betrachte für eine
  Garbe $F \in \Ens_{/|X|}$ auf $V_i(\sigma) = U_i(\sigma) \setminus
  |d_i^* \sigma|$ nur diejenigen Schnitte von $F$, die sich zu
  Schnitten über die gesamte zu $\sigma$ gehörige Zelle $|\sigma|$ von
  $|X|$ ausdehnen lassen. Wir setzen nun $p_*F(\tau)$ als die Schnitte
  über $|\tau|$, die sich für $\sigma \to \tau = d_i^*(\sigma)$ eine
  Randabbildung fortsetzen lassen zu Schnitten über $V_i(\sigma)$.


  Betrachte  zunächst für einen
  nichtdegenerierten Simplex $\sigma$ von $X$ das System von Mengen
  $\Gamma(|\sigma|, F)$ für $|\sigma| \subset |X|$ die zu $\sigma$
  gehörige Zelle der geometrischen Realisierung
  (vgl. \ref{cw-cells}). Setze dann $p_*F(\sigma)$ diejenigen Elemente
  $s \in \Gamma(|\sigma|, F)$, die sich für jeden Morphismus $\tau \to
  \sigma$ in $X \otimes R$ ausdehnen lassen zu einem Schnitt $t \in
  \Gamma(|\tau|, F)$.
  % TODO einseitig ausdehnen
\item
