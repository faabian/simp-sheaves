...
Verfügung stehen. Problematisch dabei ist, dass Aussagen über
Isomorphismen (strikte Äquivalenz) im Allgemeinen zu restriktiv sind,
und wir Limites nur bis auf (schwache) Äquivalenz von Kategorien
benötigen. Wir versuchen hier, dies durch ein Modell in den Griff zu
bekommen, welches schwache Limites als starke Limites modelliert und
dabei die Informationen über die beteiligten Isomorphismen
mitverfolgt.
\begin{defn}
  Ein Limes eines Systems von Kategorien $C_i$ ist die Kategorie $L$
  der Produktkategorie $\prod_i C_i$
\end{defn}

Dafür führen wir einen Limesbegriff für Garben- und
allgemeiner Funktorenkategorien ein.
% TODO sollte das ein Lemma sein und die univ Eig zeigen?
\begin{defn}
  Sei $I$ eine kleine Kategorie, $C: I \to \Cat$ ein System von
  kleinen Kategorien und $D$ eine Kategorie. Ein \emph{Limes von
    Funktoren} $F \in \lim_i [C_i, D]$ ist ein Tupel von Funktoren
  $F_i \in [C_i, D]$ samt Isotransformationen $T_f: F_i \Isotrafo F_j
  \circ Cf$ für alle $f: i \to j$ in $I$.
\end{defn}
Die Formulierung ist analog gebildet zu der eines Limes in $\Ens_D$,
wobei striktes Übereinstimmen (Identitätstransformation) auf schwaches
Übereinstimmen (Isotransformation) abgemildert wird. Der Begriff
gewährleistet die Stetigkeit des $\Hom$ in der schwachen ``Kategorie''
der Kategorien mit Äquivalenzen als Isomorphismen:
\[ [ \col_i C_i, D] \qiso \lim_i [C_i, D] \]
ist eine Äquivalenz von Kategorien. Man beachte, dass die
Isotransformationen $T_f$ Teil der Daten des Funktor-Limes sind.
...
\begin{satz} \label{sheaf-col}
  ...
  Limes von Funktoren
  \[ \lim_i \ins_i^*: \EnsX \to \lim_i \ins_i^* \EnsX \]
...
% TODO das muss raus oder woanders hin
\begin{bem}
...
\subsection{Realisierung als Simplexkategorie}
