...
\begin{proof}
  ...
  %% Für die umgekehrte Richtung betrachten wir die Restriktion
  %% $\Gamma(U(\sigma), F) \to F_x$. Diese ist bijektiv, falls wir jeden
  %% Keim $s \in F_x$, repräsentiert durch $s \in F(V)$ für eine
  %% o. E. zusammenhängende offene Teilmenge $V$ von $|\K|$, eindeutig zu
  %% einem Schnitt über $U(\sigma)$ fortsetzen können. Klar ist, dass für
  %% jedes $\tau \geq \sigma$ die Mengen $V \cap |\tau|$ zusammenhängende
  %% offene nichtleere Teilmengen von $|\tau|$ sind und wir nach der
  %% simplizialen Konstanz von $F$ die Einschränkungen $s|_{V \cap
  %%   |\tau|}$ zu einem Schnitt über ganz $|\tau|$ fortsetzen
  %% können. Entscheidend ist nun, dass die Fortsetzungen auch
  %% untereinander verträglich sind und einen stetigen Schnitt über
  %% $U(\sigma)$ definieren.
